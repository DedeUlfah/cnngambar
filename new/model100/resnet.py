...
cm_display = metrics.ConfusionMatrixDisplay(confusion_matrix=confusion_mtx, display_labels=os.listdir(data_dir))
cm_display.plot()
plt.show()
...
for i in range(num_classes):
    plt.plot(fpr[i], tpr[i], label='Class {} - AUC = {:.2f}'.format(class_names[i], roc_auc[i]))
...
overall_auc = roc_auc_score(y_test_bin, y_pred_bin, multi_class='ovr')
